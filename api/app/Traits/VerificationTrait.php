...
trait VerificationTrait
{
    ...
    public function createSignature(User $user){
        $key = env('APP_KEY', '');
        $expires = Carbon::now()->addMinutes($this->expire_delay)->getTimestamp();
        $signature = hash_hmac('sha256', $user->hash.'_'.$user->email.'_'.$expires, $key);

        return '?expires='.$expires.'&hash='.hash_hmac('whirlpool', $user->email, $key).'&signature='.$signature;
    }
    ...
    public function isValidRoute(Request $request, $user_id){
        $user = User::byHashOrFail($user_id);
        $key = env('APP_KEY', '');
        $expires = $request->input('expires');
        $signature = (string) $request->input('signature');


        $compare = (string) hash_hmac('sha256', $user->hash.'_'.$user->email.'_'.$expires, $key);
