...
class LogsController extends Controller
{
    ...
    public function getLogs(Request $request, $year, $month = 0, $day = 0)
    {
        ...
        if ($hasDay) {
            ...
            //         $query->whereIn('level', $levelFilter);
            //     })
            //     ->where('actor', 'ilike', '%' .$request->term.'%')
            //     ->orWhere('level', 'ilike', '%' .$request->term. '%')
            //     ->orWhere('module', 'ilike', '%' .$request->term. '%')
            //     ->selectRaw('count(*) as count')->first()->count();

            $count = $this->searchQuery($request->term)->count();
            $data = $this->searchLog($request->term, $request->limit, $request->offset);

            $result = LogsResource::collection($data);
            ...
            $result = $this->searchLog($request->term, $request->limit, $request->offset);
        }
