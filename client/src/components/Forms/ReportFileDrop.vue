<template>
  <div
    class="group relative flex flex-row gap-1 border-4 border-dashed border-slate-200 pt-10 transition-colors hover:border-slate-500/50"
  ...
  >
    ...
    >
      <div
        class="flex-row pointer-events-none absolute flex select-none items-center justify-center text-slate-500/75 transition-all gap-2"
        :class="[
          files.length <= 0 && 'inset-0',
          files.length > 0 &&
            'inset-x-0 top-0  rounded-lg border-b border-slate-500/50 px-2 py-2',
        ]"
      >
      <div class="hidden md:flex flex-col">
        <TIcon name="upload_file" size="md"></TIcon>
      </div>
        <div class="leading-none transition-all hidden md:flex flex-col">
          <span class="font-sans text-xs xl:text-sm">Drag N' Drop Files here</span>          
        </div>
        <div
          class="transition-all hidden md:flex flex-col text-xs xl:text-sm font-sans"
          :class="[files.length > 0 && 'flex-auto text-center']"
        >
          OR
        </div>
        <div
          class="cursor-pointer rounded-full border border-slate-500/50 px-2 hover:bg-slate-500/50 hover:text-white transition-all flex flex-col font-bold text-xs xl:text-sm font-sans"
    ...
    <transition-group tag="div" name="files" class="relative flex flex-col w-full">
      <div
        v-for="item in files"
        :key="item.id"
        v-show="item.progress < 100"
        class="w-full leading-none overflow-auto"
      >
        <slot
          :file="item.file"
          :readonly="disabled"
