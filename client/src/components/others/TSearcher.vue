  ...
  >
    <TCardHeader class="border-b-0 pt-3" ref="headerComponent">
      <TInput
        ...
        :label="`Search ${title}`"
        class="!pb-0"
        innerClass="bg-white text-slate-700 dark:bg-slate-300"
    ...
    <TCardBody class="min-h-[12rem] overflow-auto">
      <div class="mb-2 flex items-center text-sm leading-none">
        ...
        <!-- v-if="columns.lenght > 0 || filters.length > 0" -->
        <TButton @click="data.settings = !data.settings">
          <TIcon
            name="settings"
            size="sm"
      ...
      <div v-else class="flex flex-col gap-2 rounded-md border p-2">
        ...
        >
          ...
          >
            ...
            <div v-if="closable" class="flex items-center px-3">
              <TButton
    ...
    <TCardFooter class="flex items-center justify-end gap-2">
      <TButton
        @click="search(null)"
        class="overflow-auto rounded-full border bg-primary py-1.5 px-2 text-white shadow-md transition-colors"
        label="Search"
      >
      </TButton>
      <TButton
        @click="emit('close')"
        class="overflow-auto rounded-full py-1.5 px-2"
        label="Cancel"
      >
      </TButton>
    </TCardFooter>
